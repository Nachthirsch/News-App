...
const NewsCard = ({
  ...
  isSaved,
  isFeature = false,
  variant = "regular", // 'hero', 'feature', 'horizontal', 'regular', 'small'
  priority = false,
}) => {
  ...
  return (
    ...
    >
      ...
      <div className={`flex flex-col flex-grow ${getContentContainerClass()}`}>
        {/* Only show this source indicator for non-hero/feature variants */}
        {!(variant === "hero" || variant === "feature") && (
        ...
        <div className="flex items-center justify-between pt-3 mt-auto border-t border-gray-100 dark:border-gray-700">
          {/* For hero and feature, show reading time on the left */}
          {(variant === "hero" || variant === "feature") && <span className="text-xs text-gray-500 dark:text-gray-400">{Math.ceil(description.length / 200)} min read</span>}
          ...
          <a
            ...
            className={`text-sm font-medium text-blue-600 dark:text-blue-400
                      hover:underline inline-flex items-center ${variant === "hero" || variant === "feature" ? "" : "ml-auto"}`}
          >
            Baca selengkapnya
...
NewsCard.propTypes = {
  ...
  variant: PropTypes.oneOf(["hero", "feature", "horizontal", "regular", "small"]),
